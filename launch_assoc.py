...
class ExperimentManager:
    ...
    def launch(self, l2_assoc):
        redirection = '-re'
        graph_name = self.args.graph_name if self.args.graph else self.kronecker_size
        outdir = '--outdir=' + WHERE_AM_I + "/results/" + self.args.bench_name + "/" + graph_name + "/assoc_" + l2_assoc
        ...
        other_script_options += ' --l2cache --l2_size=4kB --l2_assoc={}'.format(l2_assoc)
        ...
        try:    
            print("\n\nStarted with l2_assoc: {} ".format(l2_assoc))
            subprocess.check_call(gem5_command, shell=True)
            print("Finished with l2_assoc: {} ".format(l2_assoc))
        except Exception as e:
            sys.exit(str(e))

def run_experiment(l2_assoc, experiment_manager, args):
    experiment_manager.launch(l2_assoc)
...
if __name__ == "__main__":
    ...
    if args.graph:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
            executor.map(method_with_args, launch_utils.l2_assocs)
    else:
