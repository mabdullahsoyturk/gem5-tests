...
class ExperimentManager:
    ...
    def launch(self, l2_replacement_policy):
        redirection = '-re'
        graph_name = self.args.graph_name if self.args.graph else self.kronecker_size
        outdir = '--outdir=' + WHERE_AM_I + "/results/" + self.args.bench_name + "/" + graph_name + "/replacement_" + l2_replacement_policy
        ...
        other_script_options += ' --l2cache --l2_size=4kB --l2-replacement-policy={}'.format(l2_replacement_policy)
        ...
        try:    
            print("\n\nStarted with l2_replacement: {} ".format(l2_replacement_policy))
            subprocess.check_call(gem5_command, shell=True)
            print("Finished with l2_replacement: {} ".format(l2_replacement_policy))
        except Exception as e:
            sys.exit(str(e))

def run_experiment(l2_replacement_policy, experiment_manager, args):
    experiment_manager.launch(l2_replacement_policy)
