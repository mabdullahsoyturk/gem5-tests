...
class ExperimentManager:
    ...
    def launch(self, l2_write_buffers):
        redirection = '-re'
        graph_name = self.args.graph_name if self.args.graph else self.kronecker_size
        outdir = '--outdir=' + WHERE_AM_I + "/results/" + self.args.bench_name + "/" + graph_name + "/write_buffer_" + l2_write_buffers
        ...
        other_script_options += ' --l1i_write_buffers={} --l1d_write_buffers={} --l2_write_buffers={}'.format(self.args.l1i_write_buffers, self.args.l1d_write_buffers, l2_write_buffers)
        ...
        try:    
            print("\n\nStarted with l2_write_buffers: {} ".format(l2_write_buffers))
            subprocess.check_call(gem5_command, shell=True)
            print("Finished with l2_write_buffers: {} ".format(l2_write_buffers))
        except Exception as e:
            sys.exit(str(e))

def run_experiment(l2_write_buffers, experiment_manager, args):
    experiment_manager.launch(l2_write_buffers)
...
if __name__ == "__main__":
    ...
    if args.graph:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
            executor.map(method_with_args, launch_utils.l2_write_buffers)
    else:
