...
class ExperimentManager:
    ...
    # build/RISCV/gem5.fast <gem5 options> configs/example/se.py --cpu-type=DerivO3CPU -n 4
    ...
    def launch(self, l2_prefecther):
        redirection = '-re'
        graph_name = self.args.graph_name if self.args.graph else self.kronecker_size
        outdir = '--outdir=' + WHERE_AM_I + "/results/" + self.args.bench_name + "/" + graph_name + "/prefetch_" + l2_prefecther + "_" + self.args.l2_replacement
        ...
        other_script_options += ' --l2cache --l2_size=4kB --l2-hwp-type={} --l2-replacement-policy={}'.format(l2_prefecther, self.args.l2_replacement)
        ...
        try:    
            print("\n\nStarted with l2_prefetcher: {} and replacement policy ".format(l2_prefecther))
            subprocess.check_call(gem5_command, shell=True)
            print("Finished with l2_prefetcher: {} ".format(l2_prefecther))
        except Exception as e:
            sys.exit(str(e))

def run_experiment(l2_prefecther, experiment_manager, args):
    experiment_manager.launch(l2_prefecther)
...
if __name__ == "__main__":
    ...
    if args.graph:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
            executor.map(method_with_args, launch_utils.l2_prefecthers)
    else:
